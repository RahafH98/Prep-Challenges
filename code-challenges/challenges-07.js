...
const cvFormatter = (arr) => {
    ...
    .map((applicant) => {
      const { firstName, lastName, tech } = applicant;
...
let cvs2 = [
  ...
  console.log(applicationsStatics(cvs2));
// -------------------------------------------------------------------------------------------------------
...
const classesAvg = (data) => {
    ...
    for (let i = 0; i < data.grades.length; i++) {
        ...
        for (let j = 0; j < grade.classes.length; j++) {
          const classData = grade.classes[j];
          const classScores = classData.classScores;
          const sum = classScores.reduce((total, score) => total + score, 0);
          const avg = Math.floor(sum / classScores.length);
          classData.avg = avg;
        }
      }
};
